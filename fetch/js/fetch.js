...
function LoadMultipleUsers(userCount){
    ...
    .then(zaidi => {
        ...
        for(let i = 0; i < zaidi.results.length; i++){
            let fn = '<div>' + zaidi.results[i].name.first + '</div>'
            let ln = '<div>' + zaidi.results[i].name.last + '</div>'
            let phone = '<div>' + zaidi.results[i].phone + '</div>'
            let img = '<img src="' + zaidi.results[i].picture.large + '">'
